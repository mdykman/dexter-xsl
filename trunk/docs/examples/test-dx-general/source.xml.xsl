...
<!-- generated by dexter from `source.xml'  -->
		...
		<xsl:element name="tests">
			...
			<xsl:element name="div"><xsl:attribute name="id"><xsl:text>group-1</xsl:text></xsl:attribute>
				<xsl:text>
		</xsl:text>
				...
				<xsl:text>
		</xsl:text><xsl:if test="string(data/found/./@value) = &quot;y&quot;">
					<xsl:element name="div"><xsl:attribute name="id"><xsl:text>test-4a</xsl:text></xsl:attribute><xsl:attribute name="name">amfound</xsl:attribute></xsl:element></xsl:if>
				<xsl:text>
		</xsl:text><xsl:if test="string(data/found/./@value) = &quot;n&quot;">
					<xsl:element name="div"><xsl:attribute name="id"><xsl:text>test-4b</xsl:text></xsl:attribute><xsl:attribute name="name">notfound</xsl:attribute></xsl:element></xsl:if>
				<xsl:text>
		</xsl:text><xsl:choose><xsl:when test="string(data/found/./@value) = &quot;y&quot;">
						<xsl:element name="div"><xsl:attribute name="cond">true</xsl:attribute><xsl:attribute name="id"><xsl:text>test-5a</xsl:text></xsl:attribute>
							<xsl:text>only one should show with 42</xsl:text></xsl:element></xsl:when><xsl:otherwise>
						<xsl:element name="div"><xsl:attribute name="cond">false</xsl:attribute><xsl:attribute name="id"><xsl:text>test-5b</xsl:text></xsl:attribute>
							<xsl:text>only one should show with 42</xsl:text></xsl:element></xsl:otherwise></xsl:choose>
				<xsl:text>
		</xsl:text><xsl:choose><xsl:when test="string(data/found/./@value) = &quot;n&quot;">
						<xsl:element name="div"><xsl:attribute name="cond">false</xsl:attribute><xsl:attribute name="id"><xsl:text>test-6a</xsl:text></xsl:attribute>
							<xsl:text>only one should show with 43</xsl:text></xsl:element></xsl:when><xsl:otherwise>
						<xsl:element name="div"><xsl:attribute name="cond">true</xsl:attribute><xsl:attribute name="id"><xsl:text>test-6b</xsl:text></xsl:attribute>
							<xsl:text>only one should show with 43</xsl:text></xsl:element></xsl:otherwise></xsl:choose>
				<xsl:text>
		</xsl:text>
			...
			<xsl:element name="div"><xsl:attribute name="id"><xsl:text>group-2</xsl:text></xsl:attribute>
				<xsl:text>
			a record
		</xsl:text><xsl:for-each select="data/record/*"><xsl:variable name="DexterDepthLevel1"><xsl:value-of select="position()"/></xsl:variable>
					<xsl:element name="div"><xsl:choose><xsl:when test="local-name(.) and ."><xsl:text>new record </xsl:text><xsl:value-of select="local-name(.)"/><xsl:text> = </xsl:text><xsl:value-of select="."/></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:element></xsl:for-each>
				<xsl:text>
