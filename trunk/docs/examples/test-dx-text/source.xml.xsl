...
<xsl:template match="/">
		...
		<xsl:apply-templates mode="af947b5bb922b52ed" select="data/found"/>
			<xsl:text>
...
<xsl:template match="data/found" mode="af947b5bb922b52ed">				<xsl:element name="div"><xsl:attribute name="id"><xsl:text>group-1</xsl:text><xsl:if test="last() &gt; 1">-<xsl:value-of select="generate-id()"/></xsl:if></xsl:attribute>					<xsl:text>
		</xsl:text>
