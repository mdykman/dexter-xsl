...
<!-- generated by dexter from `ss.html'  -->
</xsl:output><xsl:template match="/"><xsl:for-each select="data/person"><xsl:variable name="DexterDepthLevel1"><xsl:value-of select="position()"/></xsl:variable>
	...
	last </xsl:text>
				...
				<xsl:text>
</xsl:text></xsl:element></xsl:for-each></xsl:template><xsl:template name="json">
