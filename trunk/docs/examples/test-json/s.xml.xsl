...
<!-- generated by dexter from `s.xml'  -->
		...
		<xsl:element name="a">
			...
			<xsl:element name="div"><xsl:attribute name="onlick"><xsl:choose><xsl:when test="result/*"><xsl:text>return hello(</xsl:text><xsl:call-template name="js.data"><xsl:with-param name="param1" select="result/*"/></xsl:call-template><xsl:text>);</xsl:text></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute></xsl:element>
			<xsl:text>
</xsl:text></xsl:element></xsl:template><xsl:template name="js.data">
