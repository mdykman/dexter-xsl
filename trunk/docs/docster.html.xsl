<?xml version="1.0" encoding="UTF-8" standalone="no"?><xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0"><xsl:output encoding="UTF-8" indent="no" media-type="text/html" method="html">
<!-- generated by dexter from `docster.html'  -->
</xsl:output><xsl:template match="/"><xsl:element name="html"><xsl:attribute name="xmlns">http://www.w3.org/1999/xhtml</xsl:attribute>
...
<xsl:element name="div"><xsl:attribute name="class">foo</xsl:attribute>
<xsl:element name="span">
<xsl:element name="a"><xsl:attribute name="name">TOP</xsl:attribute></xsl:element></xsl:element>
...
<xsl:element name="div"><xsl:attribute name="style">clear:both; align:left; width:100%;</xsl:attribute>
 <xsl:element name="span"><xsl:attribute name="class">author</xsl:attribute><xsl:attribute name="style">width:100%  align:left;</xsl:attribute>author: <xsl:value-of select="*/author"/></xsl:element>
</xsl:element>
<xsl:element name="br"/>
...
<xsl:element name="div"><xsl:attribute name="style">float:left; clear:both;</xsl:attribute>
 ...
 <xsl:element name="p"><xsl:for-each select="*/overview/para"><xsl:element name="i"><xsl:value-of select="."/></xsl:element></xsl:for-each></xsl:element>
</xsl:element>
...
<xsl:if test="count(*/section) &gt; 1"><xsl:element name="div"><xsl:attribute name="style">top-margin:15px;</xsl:attribute>
   ...
   <xsl:element name="a"><xsl:attribute name="name">TOC</xsl:attribute></xsl:element>
      <xsl:element name="div"><xsl:attribute name="style">left-margin:30px;</xsl:attribute><xsl:for-each select="*/section">
         <xsl:element name="a"><xsl:attribute name="href"><xsl:choose><xsl:when test="title"><xsl:text><![CDATA[#]]></xsl:text><xsl:value-of select="title"/></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute><xsl:value-of select="title"/></xsl:element><xsl:element name="br"/>
...
<xsl:for-each select="*/section"><xsl:element name="div">
 <xsl:element name="a"><xsl:attribute name="name"><xsl:choose><xsl:when test="title"><xsl:text/><xsl:value-of select="title"/></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute></xsl:element>
 ...
 <xsl:element name="div"><xsl:attribute name="class">listing</xsl:attribute><xsl:for-each select="chapter">
  <xsl:element name="a"><xsl:attribute name="href"><xsl:choose><xsl:when test="../title and title"><xsl:text><![CDATA[#]]></xsl:text><xsl:value-of select="../title"/><xsl:text><![CDATA[-]]></xsl:text><xsl:value-of select="title"/></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute>&gt; <xsl:value-of select="title"/></xsl:element><xsl:element name="br"/>
 </xsl:for-each></xsl:element>
 ...
 <xsl:for-each select="chapter"><xsl:element name="div"><xsl:attribute name="class">section</xsl:attribute>
  <xsl:element name="a"><xsl:attribute name="name"><xsl:choose><xsl:when test="../title and title"><xsl:text/><xsl:value-of select="../title"/><xsl:text><![CDATA[-]]></xsl:text><xsl:value-of select="title"/></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute></xsl:element>
  ...
  <xsl:element name="div"><xsl:for-each select="*">
   <xsl:choose><xsl:when test="local-name(.) = &quot;para&quot;"><xsl:element name="p"><xsl:for-each select="link|./text()">
    <xsl:choose><xsl:when test="./self::text()"><xsl:value-of select="."/></xsl:when><xsl:when test="local-name(.) = &quot;link&quot;"><xsl:element name="a"><xsl:attribute name="href"><xsl:value-of select="@url"/></xsl:attribute><xsl:value-of select="."/></xsl:element></xsl:when></xsl:choose>
   </xsl:for-each></xsl:element></xsl:when><xsl:when test="local-name(.) = &quot;code&quot;"><xsl:element name="div">
    <xsl:element name="pre"><xsl:attribute name="style">width:80%;</xsl:attribute><xsl:value-of select="."/></xsl:element>
   </xsl:element></xsl:when></xsl:choose></xsl:for-each></xsl:element>

<xsl:element name="div"><xsl:attribute name="style">text-align:center</xsl:attribute>
  <xsl:element name="a"><xsl:attribute name="href"><xsl:choose><xsl:when test="../title"><xsl:text><![CDATA[#]]></xsl:text><xsl:value-of select="../title"/></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute><xsl:value-of select="../title"/></xsl:element>
 -- <xsl:if test="count(*/section) &gt; 1"><xsl:element name="span"><xsl:element name="a"><xsl:attribute name="href">#TOC</xsl:attribute>Table of Contents</xsl:element>-- </xsl:element></xsl:if><xsl:element name="a"><xsl:attribute name="href">#TOP</xsl:attribute>Top</xsl:element>
</xsl:element>
  <xsl:element name="br"/>
  <xsl:element name="br"/>
 </xsl:element></xsl:for-each>
