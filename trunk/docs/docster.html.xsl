...
<xsl:element name="div"><xsl:attribute name="style">clear: both; align: left; width: 100%;</xsl:attribute><xsl:element name="span"><xsl:attribute name="class">author</xsl:attribute><xsl:attribute name="style">width: 100% align : left;</xsl:attribute>author: <xsl:value-of select="*/author"/></xsl:element></xsl:element>
<xsl:element name="br"/>

<xsl:for-each select="*/references/reference"><xsl:element name="div"><xsl:attribute name="style">clear: both;</xsl:attribute>
<xsl:element name="div"><xsl:attribute name="style">padding-left: 5px; left-margin: 5px;</xsl:attribute><xsl:element name="a"><xsl:attribute name="href"><xsl:value-of select="url"/></xsl:attribute><xsl:value-of select="title"/></xsl:element><xsl:element name="br"/>
...
<xsl:for-each select="chapter"><xsl:element name="div"><xsl:attribute name="class">section</xsl:attribute><xsl:element name="a"><xsl:attribute name="name"><xsl:choose><xsl:when test="../title/text() and title/text()"><xsl:text/><xsl:value-of select="../title"/><xsl:text><![CDATA[-]]></xsl:text><xsl:value-of select="title"/></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute></xsl:element>
<xsl:element name="h3"><xsl:value-of select="title"/></xsl:element>
<xsl:element name="div"><xsl:for-each select="*">
<xsl:choose><xsl:when test="local-name(.) = &quot;para&quot;"><xsl:element name="p"><xsl:for-each select="link|./text()"><xsl:choose><xsl:when test="./self::text()"><xsl:value-of select="."/></xsl:when><xsl:when test="local-name(.) = &quot;link&quot;"><xsl:element name="a"><xsl:attribute name="href"><xsl:value-of select="@url"/></xsl:attribute><xsl:value-of select="."/></xsl:element></xsl:when></xsl:choose></xsl:for-each></xsl:element></xsl:when><xsl:when test="local-name(.) = &quot;code&quot;"><xsl:element name="div"><xsl:element name="pre"><xsl:attribute name="style">width: 80%;</xsl:attribute><xsl:value-of select="."/></xsl:element></xsl:element></xsl:when></xsl:choose></xsl:for-each></xsl:element>
