...
<!-- generated by dexter from `source.xml'  -->
		...
		<xsl:element name="div">
			<xsl:text>
	</xsl:text><xsl:for-each select="data/names/name"><xsl:variable name="DexterDepthLevel1"><xsl:value-of select="position()"/></xsl:variable>
				<xsl:element name="div"><xsl:attribute name="onlick"><xsl:choose><xsl:when test="."><xsl:text>return collect(</xsl:text><xsl:call-template name="json"><xsl:with-param name="param1" select="."/></xsl:call-template><xsl:text>);</xsl:text></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute></xsl:element></xsl:for-each>
			<xsl:text>
	---------------------------------------
	</xsl:text><xsl:for-each select="data/person"><xsl:variable name="DexterDepthLevel1"><xsl:value-of select="position()"/></xsl:variable>
				<xsl:element name="div">
					<xsl:text>
		first </xsl:text>
		...
		last </xsl:text>
					...
					<xsl:element name="a"><xsl:attribute name="href"><xsl:choose><xsl:when test="."><xsl:text>javascript:dosomething(</xsl:text><xsl:call-template name="json"><xsl:with-param name="param1" select="."/></xsl:call-template><xsl:text>)</xsl:text></xsl:when><xsl:otherwise><xsl:text/></xsl:otherwise></xsl:choose></xsl:attribute>
						<xsl:text>do something</xsl:text></xsl:element>
					<xsl:text>
</xsl:text></xsl:element></xsl:for-each>
			<xsl:text>
</xsl:text></xsl:element></xsl:template><xsl:template name="json">
	<xsl:param name="param1"/>
	<xsl:call-template name="js.data">
		<xsl:with-param name="param1" select="$param1"/>
	</xsl:call-template>
</xsl:template><xsl:template name="js.data">
	<xsl:param name="param1"/>
	<xsl:apply-templates mode="js.data" select="."/>
